...
def fit_cos_square(x, y):
    def cos_square(x, A):
        return A * np.cos(x) ** 2

    initial_guess = [1]  # theta

    popt, _ = curve_fit(cos_square, x, y, p0=initial_guess)
...
def plotting(x, y, color, x_axis, y_axis, title):
...
def fit_two_gaussians(x, y):
    def combined_gaussian(x, A1, mu1, sigma1, A2, mu2, sigma2):
        def gaussian(x, A, mu, sigma):
            return A * np.exp(-0.5 * ((x - mu) / sigma) ** 2)
...
def error_evaluation(original_curve, fitted_curve):
    err = 0
    for i in range(original_curve):
        err += abs(original_curve[i] - fitted_curve[i])
